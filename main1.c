...
void vault_menu(const char *master_password) {
  int choice;
  char site[64], account[64], pass[64], input[16];
  ...
  while (1) {
    ...
    if (choice == 1) {
      ...
      char combined[256];
      snprintf(combined, sizeof(combined), "%s|%s|%s", site, account, pass);

      unsigned char encrypted[512];
      int encrypted_len = aes_encrypt((unsigned char *)combined,
                                      strlen(combined), key, iv, encrypted);
      ...
      int lengths[100];
      unsigned char enc_data[100][512];
      while (!feof(fp) && count < 100) {
        int len;
        if (fread(&len, sizeof(int), 1, fp) != 1)
          break;
        if (fread(enc_data[count], 1, len, fp) != len)
          break;
        lengths[count] = len;

        unsigned char decrypted[256] = {0};
        aes_decrypt(enc_data[count], len, key, iv, decrypted);
        strncpy(entries[count], (char *)decrypted, sizeof(entries[count]));
        count++;
      }
      fclose(fp);
      ...
      printf("\nSelect entry number to %s: ",
             (choice == 2 ? "change" : "remove"));
      fgets(input, sizeof(input), stdin);
      int idx;
      ...
      if (choice == 2) {
        ...
        char new_combined[256];
        char *tok_site = strtok(entries[idx], "|");
        char *tok_account = strtok(NULL, "|");
        snprintf(new_combined, sizeof(new_combined), "%s|%s|%s", tok_site,
                 tok_account, pass);

        int new_len = aes_encrypt((unsigned char *)new_combined,
                                  strlen(new_combined), key, iv, enc_data[idx]);
        lengths[idx] = new_len;
        printf("[✔] Entry updated.\n");
      } else if (choice == 0) {
        for (int j = idx; j < count - 1; j++) {
          memcpy(enc_data[j], enc_data[j + 1], sizeof(enc_data[j]));
      ...
      fp = fopen(VAULT_FILE, "wb");
      for (int i = 0; i < count; i++) {
        fwrite(&lengths[i], sizeof(int), 1, fp);
        fwrite(enc_data[i], 1, lengths[i], fp);
      }
      fclose(fp);
    }
